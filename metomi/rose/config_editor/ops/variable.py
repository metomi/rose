...
class VariableOperations(object):
    ...
    def add_var(self, variable, skip_update=False, skip_undo=False):
        ...
        if v_id in [v.metadata.get('id') for v in variables]:
            ...
            if sect not in config_data.sections.now:
                ...
                group = (metomi.rose.config_editor.STACK_GROUP_ADD + "-" +
                         str(time.time()))
    ...
    def fix_var_ignored(self, variable):
        ...
        if metomi.rose.variable.IGNORED_BY_SYSTEM in ignored_reasons:
            # Doc table I_t
            if metomi.rose.config_editor.WARNING_TYPE_ENABLED in variable.error:
                # Enable new_reason_dict.
                # Doc table I_t -> E
                pass
            if metomi.rose.config_editor.WARNING_TYPE_NOT_TRIGGER in variable.error:
                pass
        elif metomi.rose.variable.IGNORED_BY_USER in ignored_reasons:
            # Doc table I_u
            if metomi.rose.config_editor.WARNING_TYPE_USER_IGNORED in variable.error:
                # Enable new_reason_dict.
                # Doc table I_u -> I_t -> *,
                #           I_u -> E -> compulsory,
            ...
            if metomi.rose.config_editor.WARNING_TYPE_ENABLED in variable.error:
                # Doc table E -> I_t -> *
                new_reason_dict = {metomi.rose.variable.IGNORED_BY_SYSTEM:
    ...
    def set_var_ignored(self, variable, new_reason_dict=None, override=False,
        ...
        if (not override and
                ...
                metomi.rose.variable.IGNORED_BY_SYSTEM not in new_reason_dict):
            if metomi.rose.config_editor.WARNING_TYPE_NOT_TRIGGER in variable.error:
                variable.error.pop(
        ...
        if len(my_ignored_keys) > len(old_ignored_keys):
            ...
            if (not old_ignored_keys and
                    metomi.rose.config_editor.WARNING_TYPE_ENABLED in variable.error):
                variable.error.pop(metomi.rose.config_editor.WARNING_TYPE_ENABLED)
        else:
    ...
    def get_var_changes(self, variable):
        ...
        if self.is_var_modified(variable):
            ...
            if variable.value != self.get_var_original_value(variable):
                return metomi.rose.config_editor.KEY_TIP_CHANGED.format(old_value)
            ...
            if len(old_ignore) > len(variable.ignored_reason):
                return metomi.rose.config_editor.KEY_TIP_ENABLED
            if (metomi.rose.variable.IGNORED_BY_SYSTEM in variable.ignored_reason and
                    metomi.rose.variable.IGNORED_BY_SYSTEM not in old_ignore):
                return metomi.rose.config_editor.KEY_TIP_TRIGGER_IGNORED
            if (metomi.rose.variable.IGNORED_BY_USER in variable.ignored_reason and
                    metomi.rose.variable.IGNORED_BY_USER not in old_ignore):
                return metomi.rose.config_editor.KEY_TIP_USER_IGNORED
            if (metomi.rose.variable.IGNORED_BY_SECTION in variable.ignored_reason and
                    metomi.rose.variable.IGNORED_BY_SECTION not in old_ignore):
