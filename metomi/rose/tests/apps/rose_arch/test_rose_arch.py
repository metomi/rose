...
ZIP_METHODS = {
    ...
    "zstdme": "zstdme.zst",
    "tarzstdme": "tarzstdme.tar.zst",
    "xzme": "xzme.xz",
    "tarxzme": "tarxzme.tar.xz",
}
...
@pytest.mark.parametrize(
    "source_file, expected", list(ZIP_METHODS.items())
)
def test_rose_arch(
    tmp_path,
...
def test_raises_too_many_threads_err(
    ...
    """Test that the target is reported as bad if an archiver does not
    support threads, but a thread number (other than 1) is passed in.
    ...
    # One bad target is reported, and nothing is archived.
    assert run_rose_arch_task() == 1
    assert not (tmp_path / 'archive' / f'zipme.{archiver}').exists()
...
def test_zstd_accepts_threads(
    tmp_path, setup_rose_arch_task, run_rose_arch_task, archiver
):
    """Test that zstd schemes accept a thread count other than 1."""
    setup_rose_arch_task(
        f"""
            mode=rose_arch
    ...
    run_rose_arch_task()
    assert (tmp_path / 'archive' / f'zipme.{archiver}').exists()
