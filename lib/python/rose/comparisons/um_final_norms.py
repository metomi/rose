...
#-----------------------------------------------------------------------------
"""Extract UM final norms from a .leave file."""

from rose.ana import DataLengthError, data_from_regexp


class UMFinalNorms(object):
    def run(self, task, variable):
        """Return final norms given the contents of a UM leave file"""
        filevar  = variable + "file"
        filename = getattr(task, filevar)
        numbers = data_from_regexp(r"Final\s*Absolute\s*Norm\s*:\s*(\S+)", 
                                   filename)
