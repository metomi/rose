...
if __name__ == "__main__":
    ...
    config_tree_loader = ConfigTreeLoader()
...
""")
        f.close()
        config_tree = config_tree_loader("t1", "rose-t.conf")

        string_io = StringIO()
        config_dumper(config_tree.node, string_io)
        test("t1.node", string_io.getvalue(), r"""title=breakfast
type=fried up
...
""")
        string_io.close()
        test("t1.files", config_tree.files, {})
        conf_dir = os.path.join(os.getcwd(), "t1")
        test("t1.conf_dirs", config_tree.conf_dirs, [conf_dir])
...
""")
        ...
        for k, v in (("sausage", "10 fat sausages"),
            ...
            f.write(v + "\n")
            f.close()
        config_tree = config_tree_loader("t2", "rose-t.conf")

        string_io = StringIO()
        config_dumper(config_tree.node, string_io)
        test("t2.node", string_io.getvalue(), r"""title=all day breakfast
...
""")
        ...
        test("t2.files", config_tree.files, {"bin/make-breakfast": conf_dir,
                                             "etc/sausage": conf_dir,
                                             "etc/bread": conf_dir,
                                             "etc/tomato": conf_dir})
        test("t2.conf_dirs", config_tree.conf_dirs, [conf_dir])
...
""")
        ...
        f.close()
        config_tree = config_tree_loader("t3", "rose-t.conf")

        string_io = StringIO()
        config_dumper(config_tree.node, string_io)
        test("t3.node", string_io.getvalue(), r"""size=large
...
""")
        ...
        test("t3.files", config_tree.files, {"bin/make-breakfast": t2_conf_dir,
                                             "etc/sausage": t2_conf_dir,
                                             "etc/bread": t3_conf_dir,
                                             "etc/tomato": t2_conf_dir})
        test("t3.conf_dirs", config_tree.conf_dirs,
...
""")
        ...
        string_io = StringIO()
        config_dumper(config_tree.node, string_io)
        test("t3_opt.node", string_io.getvalue(), r"""size=large
title=all day breakfast
type=fried up
...
""")
        ...
        f.write("50/50 slice bread\n")
        f.close()
        config_tree = config_tree_loader("t4", "rose-t.conf",
                                         conf_dir_paths=["../a"])

        string_io = StringIO()
        config_dumper(config_tree.node, string_io)
        test("t4.node", string_io.getvalue(), r"""size=large
title=all day breakfast
type=fried up
