...
for CYCLE in 20130101T0000Z 20130101T1200Z 20130102T0000Z; do
    ...
    tar -I zstd -tf $FLOW_RUN_DIR/foo/$CYCLE/hello/worlds/planet-n.tar.zst | \
...
[$CYCLE] Greet Triton
__TXT__
    TEST_KEY="$TEST_KEY_BASE-$CYCLE-jupiter-moons.tar.zst"
    tar -xf $FLOW_RUN_DIR/foo/$CYCLE/hello/worlds/jupiter-moons.tar.zst -O \
        >"$TEST_KEY.out"
...
[FAIL] foo://20130101T1200Z/hello/worlds/planet-n.tar.zst: bad command-format: foo put %(target)s %(source)s: KeyError: 'source'
[FAIL] ! foo://20130101T1200Z/hello/worlds/planet-n.tar.zst [compress=tar.zst]
...
file_cmp "${TEST_KEY}.err" "${TEST_KEY}.err" <<'__ERR__'
[FAIL] [arch:$UNKNOWN_DARK_PLANETS.tar.zst]=: configuration value error: [UNDEFINED ENVIRONMENT VARIABLE] UNKNOWN_DARK_PLANETS
__ERR__
TEST_KEY="$TEST_KEY_BASE-bad-archive-5"
sed '/^\[FAIL\] /!d' "${FILE_PREFIX}5/01/job.err" >"${TEST_KEY}.err"
file_cmp "${TEST_KEY}.err" "${TEST_KEY}.err" <<__ERR__
[FAIL] [arch:inner.tar.zst]source=hello/mercurry.txt: configuration value error: [Errno 2] No such file or directory: '${FLOW_RUN_DIR}/share/cycle/20130101T1200Z/hello/mercurry.txt'
[FAIL] ! foo://20130101T1200Z/hello/worlds/inner.tar.zst [compress=tar.zst]
...
file_cmp "$TEST_KEY.err" "$TEST_KEY.err" <<__ERR__
[FAIL] foo push foo://20130101T1200Z/hello/worlds/mars.txt.zst $FLOW_RUN_DIR/share/cycle/20130101T1200Z/hello/mars.txt # return-code=1, stderr=
[FAIL] foo: push: unknown action
[FAIL] ! foo://20130101T1200Z/hello/worlds/mars.txt.zst
[FAIL] !	hello/mars.txt (hello/mars.txt)
__ERR__
TEST_KEY="$TEST_KEY_BASE-bad-archive-7"
sed '/^\[FAIL\] /!d' "${FILE_PREFIX}7/01/job.err" >"${TEST_KEY}.err"
file_cmp "${TEST_KEY}.err" "${TEST_KEY}.err" <<'__ERR__'
[FAIL] foo://20130101T1200Z/planet-n.tar.zst: bad rename-format: %(planet?maybedwarfplanet???)s: KeyError: 'planet?maybedwarfplanet???'
__ERR__
...
file_cmp "${TEST_KEY}.err" "${TEST_KEY}.err" <<'__ERR__'
[FAIL] foo://20130101T1200Z/planet-n.tar.zst: bad rename-parser: planet-(?P<planet>[MVEJSUN]\w+.txt: error: missing ), unterminated subpattern at position 7
__ERR__
...
file_cmp "${TEST_KEY}.err" "${TEST_KEY}.err" <<__ERR__
[FAIL] ! foo://20130101T1200Z/planet-n.tar.zst
