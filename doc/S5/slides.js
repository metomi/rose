...
//
// IMPORTANT: This file has been modified to make it compatable with bootstrap
//
...
function currentSlide() {
    ...
    cs.innerHTML = '<span class="label label-default"><span id="csHere">' + snum + '<\/span> ' +
        '<span id="csSep">\/<\/span> ' +
        '<span id="csTotal">' + (smax-1) + '<\/span></span>';
    if (snum == 0) {
...
function toggle() {
    ...
    if (!slides.disabled) {
        ...
        s5mode = false;
        fontSize('14px'); // bootstrap default is 14 px
        for (var n = 0; n < smax; n++) {
        ...
        document.getElementById('toggle').innerHTML = " Exit Slideshow ";
        s5mode = true;
        fontScale();
        for (var n = 0; n < smax; n++) {
            var slide = slideColl[n];
...
function clicker(e) {
    ...
    if (target.id != 'footer' && target.id != 'navLinks') return true;
...
function createControls() {
    ...
    $('<form />', {
        ...
        $('<div />', {'id': 'navLinks'}).append(
            ...
            $('<button />', {
                'class': 'btn btn-default navButton',
                'onclick': 'go(-1);',
                'accesskey': 'z',
                'id': 'prev'
            }).append(
...
function fontScale() {  // causes layout problems in FireFox that get fixed if browser's Reload is used; same may be true of other Gecko-based browsers
    ...
    var vScale = 24;  // both yield 32 (after rounding) at 1024x768. @benfitzpatrick: changed from 22 to 27
